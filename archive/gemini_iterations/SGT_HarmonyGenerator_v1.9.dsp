...
// Test Mode now acts as the master on/off for the internal audio path.
testMode = checkbox("[4] Debug Tools/Test Tone Enable");
testFreq = hslider("[4] Debug Tools/Test Tone Freq [unit:Hz]", 440, 100, 1000, 1);

// Input source is now explicitly 0.0 (silence) or the test tone.
// This removes external audio input entirely, eliminating feedback.
...
// Latch the value and smooth the transition
...
with {
    proc_input = input_source; // The only audio source (0.0 or test tone)
    ...
    // Input Meter (Displays the currently selected test tone frequency)
    input_freq_meter = testFreq
        : hbargraph("[4] Debug Tools/Input Freq (Hz)", 0, 1000);

    // Output Meter (Displays the calculated harmony frequency)
