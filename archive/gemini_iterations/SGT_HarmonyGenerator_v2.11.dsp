...
// Extended range with tooltip warning
manual_shift_value = hslider("[1] Harmony Control/Manual Semitones [tooltip:Quality degrades beyond ±12 semitones]", 7, -24, 24, 1);
...
with {
    ...
    // Adaptive window sizing: 4096 for extreme shifts, 2048 for normal
    ...
    // Pitch shifting with adaptive window
    voice2 = proc_input : tdhs_pitch_shifter(final_shift_value, current_ola_window, olaXFadeDefault);

    // Dry/Wet mixing
    output_mix = (proc_input * (1.0 - wetDry)) + (voice2 * wetDry);

    // Anti-aliasing filter (18kHz LPF)
